...
function getInformation(){
  var cOrgid = JSON.parse(pbESYS.getDeviceJsonInfo()).jgid,
      cUserid = pbESYS.getAppCertifyInfo('PbKey_H5_Home_Auth_UserId'),
      token = pbESYS.getAppCertifyInfo('PbKey_H5_Home_Auth_Token'),
      cLoginname = pbESYS.getAppCertifyInfo('PbKey_H5_Home_Auth_LoginName');
  $.ajax({
    type: 'POST',
    ...
    success: function (data) {
      console.log(data);
      if(data.retHead === 1){
        pbESYS.writeLocalFile('reg',1,JSON.stringify(data.data));
      }else{
        console.log(data.retHead,data.desc)
      }
    },
    error: function (XMLHttpRequest, textStatus, errorThrown) {
      console.log('服务异常');
