<template>
  <ul class="notice-group" id="notice-group" style="top: 0;">
    <li v-for="notice in noticeList" class="notice-item"><a :href="'pobo:pageId=900005&url=http://static.yunyesoft.com/static/consult/#/detail?cId='+notice.detailId">{{ notice.infotitle }}</a></li>
    <template v-if="noticeList.length > 1">
      <li v-for="n in 2" class="notice-item"><a :href="'pobo:pageId=900005&url=http://static.yunyesoft.com/static/consult/#/detail?cId='+noticeList[n].detailId">{{ noticeList[n - 1].infotitle }}</a></li>
    </template>
