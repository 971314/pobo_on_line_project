    ...
    methods: {
        ...
        initial:function () {
            if(this.types == 0){
                ...
                if(this.marketU && this.codeU){
                    this.nameCode = this.codeU;
                    this.nameMarket = parseFloat(this.marketU);
                    if(window.pbE){
                        pbE.HQ().hqSubscribe(0, JSON.stringify({"1":[{2:this.marketU,3:this.codeU}]}));
                        this.GetPriceRate = pbE.HQ().hqGetPriceRate(this.codeU,this.marketU);
                        var tradeHq = JSON.parse(pbE.HQ().getHQInfo(this.codeU,this.marketU)).name;
                        if(tradeHq){
                            this.dealname = tradeHq;
                        }
                    }else{
                        this.dealnames =[{"name":"沪铜1702","market":5000,"code":'GLNG'}]
                    }
                }
            }
        }
