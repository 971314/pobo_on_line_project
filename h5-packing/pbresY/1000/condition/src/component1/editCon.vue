                ...
                <ul>
                    ...
                    <li>
                        ...
                        <span class="trustSpan">
                            <input type="text"  class="inputText" v-model="editQueryInfo.priceNameEdit"/>
                        </span>
...
//                    }
                    ...
                    if(window.pbE){
                        ...
                        if(tradeHq){
                            ...
                            _this.editQueryInfo.HYDM =  data.HQMarket;//合约市场
                            _this.editQueryInfo.contractEditCode = data.HQCode;//合约品种
                            //订阅行情
                            nameCode =  _this.editQueryInfo.contractEditCode;
                            //market\code
                            pbE.HQ().hqSubscribe(0, JSON.stringify({"1": [{2: _this.editQueryInfo.HYDM, 3:  _this.editQueryInfo.contractEditCode}]}));//订阅最新价
                            //获取行情商品价格倍数
                            priceMulriple = pbE.HQ().hqGetPriceRate( _this.editQueryInfo.contractEditCode, _this.editQueryInfo.HYDM);
                        }
                    }
                    //以下是根据类型来判断当前选中的样式
                    // 有效期 1-当日有效，2-永久有效
