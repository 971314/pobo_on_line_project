<template>
    <div style="padding-top:39px">
        <span @click="reset()" class="navThreeTitle" style="position: fixed; top: 13px; right: 10px; z-index: 1030;">重置</span>
        <div class="setDiv">
            <div class="setOut">
                <ul>
                    <li>
                    ...
                    <li v-if="initPrice">
                        ...
                        <div class="priceTwo"></div>
                        <div class="priceThree">
                            <input  type="number" placeholder="价格" v-model="setDataObj.tjPriceValue"/>
                        </div>
                    </li>
                    <li v-else="initTime">
                    ...
                    <li>
                        ...
                        <div class="conTwo">
                        <span class="market">
                             <input type="number" class="inputText"  v-model="setDataObj.Volume"/>
                        </span>
                        </div>
                    </li>
                </ul>
...
<script>
    ...
    import selectConDialog from './dialog/selectConDialog.vue';
    import priceDialog from './dialog/priceDialog.vue';
                        ...
                        for(var i = 0;i < data.length;i++){//最新价
                            if(_this.selectContractName != '选择期货合约'){
                                if(data[i][10] == nameCode){
                                    if(data[i][29]){
                                        lastPrice = pbUtils.decimalDecPrice(decimal,data[i][29]/priceMulriple) ;//最新价
                                        $("#newPrice").html(pbUtils.decimalDecPrice(decimal,data[i][29]/priceMulriple));//最新价
                                    }else{
                                        $("#newPrice").html("--");//最新价
                                    }
                                    if(data[i][30]){
                                        upPrice = pbUtils.decimalDecPrice(decimal,data[i][30]/priceMulriple);//涨停价
                                    }
                                    if(data[i][31]){
                                        downPrice = pbUtils.decimalDecPrice(decimal,data[i][30]/priceMulriple);//跌停价
                                    }
                                    if(data[i].BidSize[0][40]){
                                        opponentPriceSell = pbUtils.decimalDecPrice(decimal,data[i].BidSize[0][40]/priceMulriple);//卖
                                    }
                                    if(data[i].AskSize[0][42]){
                                        opponentPriceBuy = pbUtils.decimalDecPrice(decimal,data[i].AskSize[0][42]/priceMulriple);//买
                                    }
                                }
                            }
                        }
                ...
                if(this.$route.params.id == 3){
                    if(_this.WTInfo.wtCode && _this.WTInfo.wtMarket){
                        ...
                        if(tradeHq){
                            ...
                            decimal = pbE.HQ().hqGetPriceDecimal(_this.WTInfo.wtCode, _this.WTInfo.wtMarket);
                        }
...
//                    data.OrderAction.SmartOrder.Order.LimitPrice = _this.setDataObj.selectPriceName+'';
            ...
            selectContractClick: function (name, market, code) {
                ...
                if(window.pbE){
                    ...
                    decimal = pbE.HQ().hqGetPriceDecimal(code,market);

                    var tradeWT = pbE.HQ().hqGetWTInfo(code,market,_this.CID);//行情信息转换交易信息
                    if(tradeWT){
                        //添加时保存的值
                        var data = JSON.parse(tradeWT);
                        _this.setDataObj.contractMarket = data.WTMarket;//合约市场
