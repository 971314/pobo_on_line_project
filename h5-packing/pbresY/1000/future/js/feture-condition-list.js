...
new Vue({
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.status == 0) {
                    ...
                    _this.isConTouchListObj.attr = attr;
                }
            }, error: function () {
                console.log("服务器异常！")
            }
    ...
    //updated:function(){
    //    if (!refreshScroll) {
    //        refreshScroll = new IScroll('#refreshContent', {
    //            probeType: 2,
    //            mouseWheel: true,
    //            momentum: false
    //        });
    //        refreshScroll.on('scroll', function(refreshScroll){
    //            console.log(refreshScroll.y)
    //            if(loadingStep == 0){
    //                console.log((refreshScroll.maxScrollY) - 20 )
    //                if (refreshScroll.y < (refreshScroll.maxScrollY - 20)) {
    //                    loadingStep = 1;
    //                    $('#pullUp').html('松开加载更多');
    //                }
    //            }
    //        }.bind(this, refreshScroll));
    //        refreshScroll.on('scrollEnd',function(){
    //            if(loadingStep == 1){
    //                loadingStep = 2;
    //                $('#pullUp').html('正在加载更多');
    //                this.noneTouchClick(20);
    //            }
    //        }.bind(this));
    //    } else {
    ...
    updated:function(){
        ...
        if (this.selectContractName && _this.setDataObj.tjPriceValue && _this.setDataObj.priceName && _this.setDataObj.Volume) {
            if (window.pbE) {
                ...
                pbE.WT().wtQueryStockBuy(CID,JSON.stringify(data));//可开
            }
        }
    },
    watch:{//非空判断
    ...
    methods: {
        noneTouchClick: function (pageSize) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.status == 0) {
                        console.log(data)
                        //_this.noneConTouchList = data.data;
                        $.each(data.data, function (key, value) {
                            _this.noneTouchConID.push(value.conditionID);
                        })
                        //console.log(_this.noneTouchConID)
                        console.log(data.data.length)
                        //_this.noneTouchLen = data.data.length
                        if(data.data.length) {//如果有记录就加载
                            loadingStep = 0;
                            _this.pagenum = _this.pagenum + 1;
                            _this.noneTouchLen = _this.noneTouchLen.concat(data.data);//将数据放到数组里面然后合并每次刷新的数据
                            $('#pullUp').html('上拉加载更多');
                        } else {
                            loadingStep = 4;
                            $('#pullUp').html('已经没有更多');
        ...
        selectContractClick: function (name, market, code) {
            ...
            if (window.pbE) {
                ...
                pbE.WT().wtGeneralRequest(CID, 6014, JSON.stringify({'54': market, '63': code}));//持仓
            }
        },
        //priceTypeClick
        ...
        //失去焦点时非空判断
        opinionBlur: function () {
            //var _this = this;
            ////console.log(_this.selectContractName,_this.setDataObj.tjPriceValue,_this.setDataObj.priceName,_this.setDataObj.Volume)
            //if (_this.selectContractName && _this.setDataObj.tjPriceValue && _this.setDataObj.priceName && _this.setDataObj.Volume) {
            //    _this.blueBtn = true;
            //    _this.initAddBtn = false;
            //} else if (_this.selectContractName || _this.setDataObj.tjPriceValue || _this.setDataObj.priceName || _this.setDataObj.Volume) {
            //    _this.blueBtn = false;
            //    _this.initAddBtn = true;
            //}
        },
