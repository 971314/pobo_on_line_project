  ...
  render: function () {
    ...
    var createBtns = function(contents){
        if(contents.length<8)
            contents.push({image1:"gengduo.png", url:"pobo:uncheck=1&pageId=900005&url=main/view/index-add.html", id:"", title:"添加"});
...
function showCon() {
  ...
  if(isPoboApp) { 
    var cons;
    if(cfg.useHistory) {
      cons = pbSYS.getHomeThreeContracts();
      if(cons) {
  ...
  if(!contract || contract.length != data.length){
    ...
    for (var i = 0; i < data.length; i++) {
      if(i % 3 == 0)
        html += '<div class="marketCon clearfix' + (!i?' active':'') + '">';
...
function callback(message) {
  ...
  if (msg.moduleId == 90004) {
    ...
    ReactDOM.render(React.createElement(InfoList, { contents: newsData[newsCurrent], dir: dirStr}), document.getElementById('info-list'));
        ...
        if(cfg.isShow) {
            ...
            if(cfg.data.length > 1){
                container.find(".area-title i").on("click", function(){
                    ...
                    newsCurrent = index;
                    if(newsData[index])
                        ReactDOM.render(React.createElement(InfoList, { contents: newsData[index], dir: 'pobo:info/' }), document.getElementById('info-list'));
                    else
...
var contract, contractInterval, prevCustom, navScroll, newsData, newsCurrent, mainConfig, prevConfig;
