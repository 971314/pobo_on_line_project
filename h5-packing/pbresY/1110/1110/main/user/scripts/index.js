...
define('user/scripts/index.js', function(require, exports, module) {
	...
	function getOptional() {
		...
		if(isPoboApp) {
			...
			if(typeof cons_ZX == "undefined") {
				bool = false;
				cons = JSON.parse(cons_ZLHY);
			} else {
				cons = JSON.parse(cons_ZX).concat(JSON.parse(cons_ZLHY));
			}
		...
		commService.getBannerAdvertise(null, function(resultVo) {
			...
			if(resultVo.error_no == 0) {
				...
				$.each(data, function(e) {
					if(data[e].url == ''){
						banner_html += "<li><img src=" + $.config.global.pictureUrl + data[e].picture + "/></li>";
					}else {
				...
				$.each($(pageId + " #home_scroller").find("img"), function() {
			...
			if(resultVo.error_no == 0) {
				...
				$.each(data, function() {
					if(notice_index++ > 2) {
						...
						notice_html += "<li href='pobo:uncheck=1&pageId=900005&url=main/information/views/details.html?url_ids=10401&Pcategory_id=noticeIndex&category_id=noticeIndex&newsid=" + this.id + "'><p>" + this.news_title + "</p><span class='time'>" + common.timeBucket(this.news_date) + "</span></li>";
					}
				});
	...
	function bindPageEvent() {
		$.bindEvent($(pageId + " #information"), function() {
			window.open("pobo:uncheck=1&pageId=900005&url=main/information/views/index.html");
		});
		...
		$.bindEvent($(pageId + " #announcement"), function() {
			window.open("pobo:uncheck=1&pageId=900005&url=main/information/views/index.html?ids=10401&Pcategory_id=noticeIndex&category_id=noticeIndex");
		})
	}
