...
var orgCode = "1998";//机构号
var numRex =/^\d+(\.\d+)?$/;//校验包含整数和小数点
var loginClass = '8';//登录类别
...
if(window.pbE)
{
   ...
   loginAccount = pbE.WT().wtGetCurrentAccountInfo("PbKey_Trade_Login_Account")||'2501';//登录账号
   accountClass =pbE.WT().wtGetCurrentAccountInfo("PbKey_Trade_Login_Account_Type") || '9';//账号类别
    ...
    orgCode = JSON.parse(pbE.SYS().getDeviceJsonInfo()).jgid;//机构编号
}
else
{
    loginAccount = '2501';//登录账号
    accountClass = '9';//账号类别
}
//原生的显示转圈方法
