...
!(function () {
   ...
   if (isPoboApp) {
    ...
    pbApi.sys_hideCircleView= function () {//隐藏全屏转圈
        pblog.info("sys_hideCircleView");
        var hideCircle = {};
        if (isPoboApp) {
            hideCircle.Trade = this.pbSYS.hideCircleView('Pbkey_Trade_WT_Circle');//交易委托转圈
            hideCircle.Default = this.pbSYS.hideCircleView('Pbkey_Default_Circle');//默认转圈
        }
        return hideCircle;
    };
    /***********系统类 End*************/
    ...
    pbApi.wt_queryEntrustRe = function(){
        var CID =  pbApi.wt_getCurrentConnectionCID();
        if(isPoboApp)
            return pbWT.wtQueryEntrustRe(CID)
    };

    pbApi.wt_queryBargainRe = function () {
        var CID = pbApi.wt_getCurrentConnectionCID();
        pblog.info('wt_wtQueryBargainRe');
        if (isPoboApp) return pbWT.wtQueryBargainRe(CID);
    };

    pbApi.wt_loginOut = function () {//交易退出
        var CID = pbApi.wt_getCurrentConnectionCID();
        if (isPoboApp) return pbWT.wtLoginOut(CID);
    };

    pbApi.wt_setCurrentAccLoginOutState = function () {
        if (isPoboApp) return pbWT.wtSetCurrentAccLoginOutState();
    };

    pbApi.wt_encrypt = function (str) {//加密
        var CID = pbApi.wt_getCurrentConnectionCID();
        if (isPoboApp) return pbWT.wtEncrypt(CID,str);
    };
