...
$(function () {
    var option = {
        callbacks: [{fun: 6012, module: 90002, callback: back6012},
            {fun: 56004, module: 90002, callback: back56004},
            {fun: 56005, module: 90002, callback: backFresh},
            {fun: 56006, module: 90002, callback: backFresh},
            {fun: 56014, module: 90002, callback: backFresh},
            {module: 90000, callback: back9000}],
...
//持仓的其他信息 position, marketInfo, asset;
    var marketInfo,
        ...
        asset,
        refreshFlag = 1;
    var CID = 3;//pbE.WT().wtGetCurrentConnectionCID();
//平仓浮动盈亏
...
//交易组件推送时更新页面
    ...
    function tradePart(CID, isNeedRender) {
        ...
        if (CONTENTS1) {
            ...
            if (position.length > 0) {
                ...
                for (var i = 0; i < position.length; i++) {
                    ...
                    //获取行情商品价格倍数
                    handNum = pbApi.wt_getMSSL(code, market); //
                    //合约代码
                    ...
                    if (!position[i]['flag']) {
                        ...
                        if (position[i]['price'] && !isNaN(position[i]['price'])) {
                            ...
                            position[i]['profit'] = (position[i]['profit'] - 0).toFixed(2);
                            position[i]['price'] = '--';
                            pblog.info(position[i]['base'] + ' 1trad base ' + i);
                            pblog.info(position[i]['136'] + ' 1trad 136' + i);
                            pblog.info(position[i]['135'] + ' 1trad 135' + i);
                            pblog.info(handNum + ' 1hq handNum');
                        }
                        pblog.info(position[i]['profit'] + 'trad ' + i)
    ...
    function calcProfit(positions, index, handNum) {
        ...
        if (currP['503'] == '1') {
            //今仓，现价-开仓均价
            if (currP['price'] && !isNaN(currP['price'])) {
            ...
            //昨仓，现价-基准价
            if (currP['price'] && !isNaN(currP['price']) && currP['base'] && !isNaN(currP['base'])) {
        ...
        for (var i = 0, l = stockInfo.length; i < l; i++) {
            ...
            if (!position[i]['flag']) {
                for (var j = 0, ll = data.length; j < ll; j++) {
                    if (stockInfo[i]['1'] == data[j]['10']) {
                        ...
                        if (data[j]['29'] && !isNaN(data[j]['29'])) {
                            ...
                            if (position[i]['base'] && !isNaN(position[i]['base'])) {
                                //接口没有拿到现价，有基准价就用基准价计算盈亏，现价显示'--'
                                position[i]['profit'] = ((position[i]['base'] - position[i]['136'] - 0).toFixed(4) - 0) * position[i]['135'] * handNum;
                                pblog.info(position[i]['base'] + ' 1hq base ' + i);
                                pblog.info(position[i]['136'] + ' 1hq 136' + i);
                                pblog.info(position[i]['135'] + ' 1hq 135' + i);
                                pblog.info(handNum + ' 1hq handNum');
                                if (position[i]['112'] == '1') {
                                    position[i]['profit'] = 0 - position[i]['profit'];
                                }
                    ...
                    pblog.info(position[i]['profit'] + 'h1 ' + i);
                    break;
                }
            } else if (position[i]['flag']) {
                position[i]['profit'] = '--';
    ...
    function backFresh(msg) {
        pbPage.fresh();
    }

    function back9000(msg) {
        totalPL = 0;
        hqPart(msg);
    }
