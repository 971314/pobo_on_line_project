...
var option = {
    callbacks:[
        {fun:6200,callback:call6200},
        {fun:6203,callback:call6203},
        {fun:6202,callback:call0201},
        {fun:6201,callback:call0201}
    ]
};
...
function confirmIn() {
    ...
    if (!$('#moneyPwd').hasClass('hide') && !$('#bankPwd').hasClass('hide')) {
        if (JTrim(obj1) == '') {
            ...
            fundPwd = pbApi.wt_encrypt($('#inFundPwd').val());
            bankPwd = pbApi.wt_encrypt($('#inBankPwd').val());
        }
...
function completeIn() {
    ...
    pbApi.wt_generalRequest(6202, JSON.stringify(data));
}
...
function completeOut() {
    ...
    pbApi.wt_generalRequest(6201, JSON.stringify(data));
}
...
function queryBalance() {  //点击查询按钮
    ...
    if (!$('#moneyPwd').hasClass('hide') && $('#bankPwd').hasClass('hide')) {
        if (!$('#money-pwd').val()) {  //没有输入资金密码，提示错误
            ...
            fundPwd = pbApi.wt_encrypt($('#money-pwd').val());
        }
        ...
        if (!$('#bank-pwd').val()) {  //没有输入银行密码
            ...
            fundPwd = pbApi.wt_encrypt($('#money-pwd').val());
            bankPwd = pbApi.wt_encrypt($('#bank-pwd').val());
        }
    ...
    pbApi.wt_generalRequest(6203, JSON.stringify(data));
}
...
function bankBalanceView() {
    ...
    pbApi.wt_generalRequest(6200, JSON.stringify({}));
}
//214银行账号，215银行编码，216银行名称，56开通币种，51资金账号
    ...
    window.setLiBank = function (data) {
        ...
        for (var acc in account) {
            if (k == 0) {
                ...
                domStr += '<li value="' + acc + '" data-code="' + account[acc]['215'] + '" data-name="' + account[acc]['216'] + '" data-currency="' + account[acc]['56'] + '" data-account="' + account[acc]['51'] + '" center-code="' + account[acc]['353'] + '" balnceFlag="' + account[acc]['219'] + '" bank-account="' + account[acc]['214'] + '" class="hover">' + '<span class="b1">' + account[acc]['216'] + '<span class="d8">' + '(尾号' + tail + ')' + '</span></span>';
                +'</li>';
                ...
                var tail = acc.slice(-4);
                domStr += '<li value="' + acc + '" data-code="' + account[acc]['215'] + '" data-name="' + account[acc]['216'] + '" data-currency="' + account[acc]['56'] + '" data-account="' + account[acc]['51'] + '" center-code="' + account[acc]['353'] + '" balnceFlag="' + account[acc]['219'] + '" bank-account="' + account[acc]['214'] + '">' + '<span class="b1">' + account[acc]['216'] + '<span class="d8">' + '(尾号' + tail + ')' + '</span></span>';
                +'</li>';
                k++;
            }
        }
