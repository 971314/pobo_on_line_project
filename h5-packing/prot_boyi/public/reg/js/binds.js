        ...
        if(msg.functionNO == 116){
            ...
            if(data.length == 0) {
                ...
                for(var i=0;i<data.length;i++){
                    p = data[i][8].split(",");
                    html += '<li><a href="reg-unbind.html?' + data[i][4] + '&' + data[i][5] + '"><span class="t">' + acctType(p[1]) + '账号</span><br><span>' + m2(p[0]) + '</span>' + m(data[i][4]) 
                    + '</a></li>';
                }
                $("#list").html(html).show();
            }
...
function listView() {
    window.reload = function(){
        pbMSG.ytzCloudRequestOtherAccountBindRelateQuery(jgid, "");    
    }
    reload();
}
...
function unbindView() {
    ...
    $("#mask a").eq(0).on("click", function(){
        pbMSG.ytzCloudRequestBindOtherAccount(params[0], params[1]-0, jgid, 2, "");
    })
}